...
public class EmployeeController {
	...
	public ModelAndView RegiterEmployee(HttpServletRequest req,HttpServletResponse res,Model model)
	{
		...
		sal.setIfsccode(Encrypt.encrypt(req.getParameter("ifsc"), MyKey.getKey()));
		sal.setAccno(Encrypt.encrypt(req.getParameter("accno"), MyKey.getKey()));
		...
		emp.setAdhar(Encrypt.encrypt(req.getParameter("adharno"), MyKey.getKey()));
		emp.setPanno(Encrypt.encrypt(req.getParameter("panno"), MyKey.getKey()));
		emp.setDept(dept);
		emp.setSal(sal);
		...
		user.setPassword(Encrypt.encrypt(otp, MyKey.getKey()));
		user.setEmp(emp);
		...
		{
			//sending mail
			String msg="Hello,MR."+emp.getName().getFname()+" your otp is:"+otp+" you can login with user employee id as username.";
